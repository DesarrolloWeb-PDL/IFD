...
function validar(formulario,mandar) {
	var campos  = formulario.getElementsByTagName("input");		//ej de llamado  <form  id="Form1" action="altamovdetallado.php" method="post" onsubmit="return validar(Form1,true)" class="form-horizontal"   role="form" >  	alert ("eeeeeeeeeeeeeeeeeeee");																				
	var listaErrores = document.getElementById("lista-errores");
	limpiarNodo(listaErrores);
	modificado = esModificado();
	longitud = campos.length;	/* Cantidad de campos del Formulario */
...
//	this.campo.value = campo.value;
   ...
   // El primer caracter del nombre determina el tipo	this.tipo = this.campo.name.charAt(0).toLowerCase();
	this.error = false;
}
